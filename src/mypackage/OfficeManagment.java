...
public class OfficeManagment {
	public static boolean check (String id , String pwd) {
		...
		try {
			ps = con.prepareStatement("select * from OfficeLogin where ID = ? and PWD=?");
			ps.setString(1,id);
			ps.setString(2,pwd);
	...
	public static int deductBal (String id , int bal , int cost) {
		Connection con=Jdbc.getCon();
		PreparedStatement ps;
		int rem=bal;
		...
		try {
			ps=con.prepareStatement("insert into OfficeTransac values(?,?,?,?)");
			ps.setString(1,TID);
			ps.setString(2,id);
			ps.setInt(3, bal-rem);
			ps.setString(2,"In Some Office Work");
		}
		catch (SQLException e) {
			System.out.println("SQL exception error while updating Transactions\n");
		}
